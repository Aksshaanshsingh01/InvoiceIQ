...
def test_purchase_invoice():
    """
    Verify all important fields extracted from the
    Purchase Invoice.
    """

    invoice = extract_invoice(PURCHASE_PDF)
    ...
    assert invoice.invoice_date == "24/08/2026"

    assert invoice.due_date == "24/08/2026"
...
def test_sale_invoice():
    ...
    invoice = extract_invoice(SALE_PDF)
    ...
    assert invoice.invoice_date == "25/08/2026"

    assert invoice.due_date == "14/09/2026"
...
def test_sample_files_exist():
    """
    Make sure our test PDFs actually exist.
    """

    assert PURCHASE_PDF.exists(), (
        f"Purchase PDF not found: {PURCHASE_PDF}"
    )

    assert SALE_PDF.exists(), (
        f"Sale PDF not found: {SALE_PDF}"
    )